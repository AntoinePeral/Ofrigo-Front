...
export const filterIngredient = (onChangeInput, listIngredient) => ({
  ...
  onChangeInput,
  listIngredient,
  listFilter: listIngredient.filter((element) =>
    element.label.toLowerCase().includes(onChangeInput.toLowerCase())
  ),
});
...
export const RemoveAnIngredientFromList = (
  ingedientToRemove,
  proposedIngredient
) => ({
  type: REMOVEANINGREDIENTFROMLIST,
  proposedIngredient: proposedIngredient.filter(
    (ingredient) => ingredient !== ingedientToRemove
  ),
});
